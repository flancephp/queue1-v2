...
{
	echo $price ? number_format($price, $decimal).' $' : '';
}

function getNumFormtPrice($price, $decimal=4)
{
	return  $price ? number_format($price, $decimal).' $' : '';
}
...
function getRevenueTotals($outLetId, $fromDate, $toDate)
{
	...
	while( $dailyImportedData = mysqli_fetch_array($result) )
	{
		...
		if($itemType == 3)
		{
			...
			$usageTotalAmount += ( ($dailyImportedData['openStock']*$itemLastPrice) + ($dailyImportedData['openStock']*$itemLastPrice) )-($dailyImportedData['closeStock']*$itemLastPrice);//usage total
	...
	return [
			...
			'cost' => ($openStockAmt+$issueInAmt)-($closeStockAmt),
