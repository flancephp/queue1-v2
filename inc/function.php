...
$runningTaskArr = [
	...
	['title' => 'Confirm', 'name' => 'confirm_ord_req']
...
//get permission details for main nav links
$checkPermission = [];
if (isset($_SESSION['designation_id'])  && isset($_SESSION['accountId'])) {
...
function getItemHistory($pId, $params = array())
{
	...
	return [
		'resRows' => $rows,
		'issuedInTot' => $issuedInTot,
		'issuedOutTot' => $issuedOutTot,
		'variancesTot' => $variancesTot,
		'issuedInQtyTot' => $issuedInQtyTot,
		'issuedOutQtyTot' => $issuedOutQtyTot,
		'variancesQtyTot' => $variancesQtyTot,
		'variancesTot' => $variancesTot,
		'cond' => $cond,
		'getId' => $getId,
		'storeName' => $storeName,
		...
		'suppName' => $suppName,
		'convertedTot' => $convertedTot,
		'convertedQtyTot' => $convertedQtyTot
	];
}
...
function insertCustomCharges($ordId, $currencyId, $otherCurAmt, $ordAmt, $noteArr, $supplierId)
{
	...
	if (isset($_SESSION['itemChargesOrd'][1]) && count($_SESSION['itemChargesOrd'][1]) > 0) //custom charge items
	{
	...
	if (isset($_SESSION['itemChargesOrd'][3]) && count($_SESSION['itemChargesOrd'][3]) > 0) //add order charges 
	{
...
function insertReqCustomCharges($ordId, $ordAmt, $noteArr, $deptId)
{
	...
	$chargesTot = 0;
	if (isset($_SESSION['itemChargesReq'][1]) && count($_SESSION['itemChargesReq'][1]) > 0) //custom charge items
	{
	...
	if (isset($_SESSION['itemChargesReq'][3]) && count($_SESSION['itemChargesReq'][3]) > 0) //add order charges 
	{
...
function getOrderPaymentLink($orderId)
{
    ...
    <a class="supPayDtl nrmPayDtl"
        href="supplierPaymentDetail.php?page=history&action=pay&orderId=<?php //echo $orderId;
																		?>"><?php //echo showOtherLangText('Pay') 
																			?></a>
</div>
<div style="border: none; background: transparent; width: 19%;">
	...
	?>
		...
		<div class="cnfrm text-center d-flex justify-content-center align-items-center pyinvBtn" style="background: #7a89fe21;">
			<a href="supplierSuccessPayment.php?page=history&action=p&orderId=<?php echo $orderId; ?>">
				<p class="h3"><?php echo showOtherLangText('Pay') ?></p>
			</a>
		</div>
...
function get_confirmOrder_permission($orderType, $designation_id, $accountId, $orderId, $orderStatus)
{

	global $con;

	$sql = " SELECT * FROM tbl_designation_sub_section_permission WHERE designation_id = '" . $designation_id . "' AND designation_section_permission_id = '3' AND account_id = '" . $accountId . "' AND type = 'confirm_ord_req' ";
	$editOrderPermissionRes = mysqli_query($con, $sql);
	$editOrderPermissionRow = mysqli_fetch_array($editOrderPermissionRes);

	$confirmVal = $orderType == 1 ? 1 : 3;
	if ((($orderType == 1 && $orderStatus == 0) || ($orderType == 2 && $orderStatus == 1))  && $editOrderPermissionRow['type_id'] == 1) {
		echo '<a href="runningOrders.php?orderId=' . $orderId . '&confirm=' . $confirmVal . '" class="cnfrm text-center d-flex justify-content-center align-items-center doc__btn complete">
				<span class="runLink">
					<!-- <span class="isuOut"></span> -->
					<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 24">
						<path d="m9.5 10 3.3 2.4q.7.6 1.3-.1L20.5 5" stroke="#8C8FA7" stroke-width="2" stroke-linecap="round" />
						<path d="M21.5 12a9 9 0 1 1-6.7-8.7" stroke="#8C8FA7" stroke-width="2" stroke-linecap="round" />
					</svg>
					<p class="btn2 cn-btn">' . showOtherLangText('Confirm') . '</p>
		</span>
		</a>';
	} else {
		echo '<span style="display: inline-block;width: 23%;">&nbsp;</span>';
...
function get_assignOrder_permission($designation_id, $accountId, $orderId, $orderType)
{
	...
	if ($editOrderPermissionRow['type_id'] == 1) {
		...
		if ($orderType == 1) {
			...
			<span>
				<svg fill="none" xmlns="http://www.w3.org/2000/svg" width="12" height="19" viewBox="0 0 15 21">
					<path d="M12.8 19.5a1 1 0 1 0 2 0zm1-6h1v-.2l-.1-.1zm-12-2-.4.9 1.7 1 .5-1zm13 8v-6h-2v6zm-.1-6.3C13.6 9.8 11.1 8 8.5 7.8A7 7 0 0 0 2 11.5l1.7 1c1-2 3-2.8 4.8-2.7 1.7.2 3.5 1.4 4.4 4z" fill="#8C8FA7" />
					<circle cx="8" cy="3.8" r="2.3" stroke="#8C8FA7" stroke-width="2" />
					<path d="M1.3 17h7m-2.5-2.5 2.1 2.1q.3.4 0 .8l-2.2 2.1" stroke="#8C8FA7" stroke-width="1.5" stroke-linecap="round" />
				</svg>
	...
	if ($issueOutPermissionRow['type_id'] == 1) {
		...
		<a href="javascript:void(0)" onclick="cnfIssueOut(<?php echo $orderId ?>)" data-bs-toggle="modal" data-bs-target="#issue-out" class="cnfrm text-center d-flex justify-content-center align-items-center doc__btn receive">
			<span class="editicon runLink edt_CatLnk">
				<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 24">
					<path d="M3.9 9q0-.5.6-.4l7 1.5q.4.1.4.5l-1 5-.2.3.3.4-.3-.4-1 .7q0 .2-.4.1L3 15.4l-.3-.6z" stroke="#8C8FA7" />
					<circle cx="12.8" cy="20.7" r="1.8" stroke="#8C8FA7" stroke-width="2" />
					<path d="M1.8 16.9a1 1 0 1 0-.4 2zm-.4 2 8 1.8.5-2L1.8 17zM20.6 6.4a1 1 0 0 0 0-2zm-8 9.8-.2 1 2 .5.2-1zm8-11.8h-2.8v2h2.8zm-5.7 2.3-2.3 9.5 2 .5 2.3-9.5zm3-2.3a3 3 0 0 0-3 2.3l2 .5a1 1 0 0 1 1-.8z" fill="#8C8FA7" />
					<path d="M14 3.5H6" stroke="#8C8FA7" stroke-width="2" />
					<path d="m4 3.5 5 2.9V.6zM14 3H8.7v1H14z" fill="#8C8FA7" />
				</svg>
	...
	if ($editOrderPermissionRow['type_id'] == 1) {
		...
		?>
			<!-- | <a title="<?php //echo showOtherLangText('Delete') 
								?>" href="runningOrders.php?canId=<?php echo $orderId; ?>&type=1" style="color:#808080;width: 8%;display: inline-block;text-align: center;" class="glyphicon glyphicon-trash" onClick="return confirm('<?php echo showOtherLangText('Are you sure to cancel this Order?') ?>')"></a> -->
    ...
    <a href="javascript:void(0)" onClick="getDelNumb('<?php echo $orderId; ?>', '1');" class="runLink">
        <span class="dlTe"></span>
			...
			<a href="javascript:void(0)" onClick="getDelNumb('<?php echo $orderId; ?>', '1');" class="dlt-bx text-center d-flex justify-content-center align-items-center doc__btn delete">
				<span class="runLink">
					<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 24">
						<path d="M10.5 15v-3m4 3v-3m-11-5h18v0q-1.3 0-1.8.2a2 2 0 0 0-1 1q-.2.5-.2 1.8v6q.1 2.7-.6 3.4-.7.8-3.4.6h-4q-2.7.1-3.4-.6T6.5 16v-6q0-1.4-.2-1.8a2 2 0 0 0-1-1Q4.8 7 3.5 7zm7.1-3.6.7-.3a7 7 0 0 1 2.4 0q.6 0 .7.3" stroke="#8C8FA7" stroke-width="2" stroke-linecap="round" />
					</svg>
				</span>
			</a>
...
function getSetupMenuPerByType($designation_id, $accountId, $type)
{

	global $con;

	$sql = " SELECT * FROM tbl_designation_sub_section_permission WHERE designation_id = '" . $designation_id . "'
	 AND designation_section_permission_id = '8' AND account_id = '" . $accountId . "' AND type = '" . $type . "' AND type_id=1 ";
	$accessPermission = mysqli_query($con, $sql);
	$accessPermissionRow = mysqli_fetch_array($accessPermission);
...
function get_all_order_action_of_order_type($orderType, $orderStatus, $checkOrdAssing, $timeTrackDet, $orderId, $designation_id, $accountId, $orderReceivedByMobUser = 0)
{
	...
	if ($orderType == 1) { //order
		...
		<?php
			...
			get_confirmOrder_permission($orderType, $designation_id, $accountId, $orderId, $orderStatus);
		?>
...
function get_all_order_action_of_requisition_type($orderType, $orderStatus, $checkOrdAssing, $timeTrackDet, $orderId, $designation_id, $accountId)
{
	...
	if ($orderType == 2) //recusition
	{
		...
		?>
			<!-- <a style="width: 21%;display: inline-block;text-align: center;" href="javascript:void(0)" class="issueOutBtn" onclick="cnfIssueOut(<?php echo $orderId ?>)"><?php //echo showOtherLangText('Issue Out') 
																																												?></a> -->
		<?Php
...
function get_edit_doctype_action_of_requisition_type($orderType, $orderStatus, $checkOrdAssing, $timeTrackDet, $orderId, $designation_id, $accountId)
{
	...
	if ($orderType == 2) //recusition
	{
		...
		<?Php
			...
			//get_issueOut_permission($designation_id, $accountId, $orderId);
		?>
			<!-- <a style="width: 20%;display: inline-block;text-align: center;" href="javascript:void(0)" class="issueOutBtn" onclick="cnfIssueOut(<?php echo $orderId ?>)"><?php //echo showOtherLangText('Issue Out') 
																																												?></a> -->
    ...
    href="runningOrders.php?orderId=<?php //echo $orderId 
									?>&confirm=3"><?php //echo showOtherLangText('Confirm') 
													?></a> -->
...
function getRevenueTotalsDemo($outLetId, $fromDate, $toDate)
{
	...
	if ($fromDate && $toDate) {
		$sql = "SELECT  o.pId, o.outLetId, o.itemType outletItemType, o.minQty outletMinQty,
		 				o.maxQty outletMaxQty, o.factor, o.notes, p.itemName, p.barCode saleBarcode, p.imgName,
		  				p.stockPrice itemLastPrice,	i.*
		...
		while ($rowOfReport = mysqli_fetch_array($outLetItemsForCountsTotals)) {
			...
			$outletAllItemsArr[$barCode] = $rowOfReport;
	...
	$dailyImportQrysql = "select SUM(sales) totalsSalesAmt, SUM(guests) guestsTotal FROM tbl_daily_import 
	...
	$guestsTotal = ($dailyImportArr['guestsTotal'] ? $dailyImportArr['guestsTotal'] : 0);
	...
	$x = 0;
	foreach ($outletAllItemsArr as $row) {
		$x++;
...
function getAccountIds($accountId, $condWithoutGroup = '')
{
	global $con;
...
function addOutletDataDateWise($accountId, $revId, $outLetId)
{
	...
	$sql = "SELECT  i.*
				...
				INNER JOIN   tbl_daily_import_items i ON(p.barCode = i.barCode) AND (p.account_id = i.account_id) AND i.outLetId=o.outLetId 

			WHERE i.outLetId = '" . $outLetId . "' AND o.outLetId = '" . $outLetId . "' AND i.account_id = '" . $accountId . "'     order by i.importedOn asc   ";
	$outLetItemsQry = mysqli_query($con, $sql);
	...
	$dateWiseOutletData = [];
	$x = 0;
	while ($dailyImportedData = mysqli_fetch_array($outLetItemsQry)) {
		$x++;

		$date = $dailyImportedData['importedOn'];

		if (!isset($dateWiseOutletData[$date])) {
		...
		$stockPrice = $dailyImportedData['stockPrice'];
		$stockPrice = $stockPrice > 0 ? $stockPrice : $dailyImportedData['lastPrice'];
		...
		$stockPrice = $stockPrice > 0 ? $stockPrice : $dailyImportedData['lastPrice']; //if still its zero then assign from product

		$usageCloseStock = $adjForEnptyBottle == 1 ? ($closeStock + $adjustment) : $closeStock; //in case of empty bottle adjustment

		$usage = ($closeStockDone) ? (($openStock + $issueIn + $barControl) - ($usageCloseStock)) : ''; //(Open Stock+Issue In) - (Close Stock)

		/*echo 'closeStockDone=' . $closeStockDone . 'openStock=' . $openStock . 'openStock=' . $issueIn . 'barControl=' . $barControl . 'usageCloseStock=' . $usageCloseStock;

			echo '<br>';

			echo 'usage=' . $usage . 'stockPrice=' . $stockPrice;
			echo '<br>';*/

		if ($dailyImportedData['itemType'] == 2) //for non usage type 1 and usage type 2. In case of usage variance will be 0
		{
			$varience = ''; //(Sales+Adjustment) - (Usage )
			$varienceAmt = '';
		} else {
		...
		$dateWiseOutletData[$date] = ['importedOn' => $date, 'usageAmtTot' => $usageAmtTot, 'varienceTotalAmt' => $varienceTotalAmt, 'salesAmtTot' => $salesAmtTot];
	} //end while loop



	foreach ($dateWiseOutletData as $outLetReport) {

		$sql = "INSERT INTO `revenue_center_outletreport_new` 
				...
				`revId` = '" . $revId . "',
				`outLetId`  = '" . $outLetId . "',
				`sales`  = '" . $outLetReport['salesAmtTot'] . "',
				`cost`  = '" . $outLetReport['usageAmtTot'] . "',
				`variance`  = '" .  $outLetReport['varienceTotalAmt'] . "',
				`createdDate`  = '" . $outLetReport['importedOn'] . "'	";
...
function getRevenueTotals_new($accountId, $outLetId, $fromDate, $toDate)
{
	global $con;
	...
	$sql = "SELECT   SUM(cost) usageTotal, SUM(variance) varienceTotal  FROM revenue_center_outletreport_new 	WHERE outLetId = '" . $outLetId . "'  AND account_id = '" . $accountId . "' AND 
			(createdDate BETWEEN  '" . $fromDate . "' AND  '" . $toDate . "')  GROUP BY  outLetId ";
	$outLetItemsQry = mysqli_query($con, $sql);




	$result =  mysqli_fetch_array($outLetItemsQry);

	if (!$result) {
		$result = [];

		$result['usageTotal'] = 0;
		$result['varienceTotal'] = 0;
	}

	return $result;
}

function getEasySalesAndGuestData($accountId, $outLetId, $fromDate, $toDate)
{
	...
	//get Totals of easy sales amount
	$dailyImportQrysql = "select SUM(sales) totalsSalesAmt, guests FROM tbl_daily_import 
								WHERE  outLetId = '" . $outLetId . "' AND account_id = '" . $accountId . "' AND importDate	 BETWEEN '" . date('Y-m-d', strtotime($fromDate)) . "' AND '" . date('Y-m-d', strtotime($toDate)) . "' GROUP BY outLetId  ";
	$dailyImportQry = mysqli_query($con, $dailyImportQrysql);
	...
	if (!$dailyImportArr) {
		...
		return ['totalsSalesAmt' => 0, 'guests' => 0];
	}

	return $dailyImportArr;
}
