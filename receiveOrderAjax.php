...
if (isset($_POST['pId']) && $_POST['pId'] > 0  && $_SESSION['id'] > 0) {
	...
	while ($taxRow = mysqli_fetch_array($ordQry)) {
		...
		$resHtml .= '<div class="price justify-content-between taxRow">
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-end curRow">  <div class="p-2"><p>' . getNumFormtPrice($taxPerChargesTotal, $getDefCurDet['curCode']) . '</p>
                                                    </div>';

		if ($_POST['curAmtVal']) {
			$resHtml .= '<div class="p-2 otherCurr"> <p> ' . showOtherCur($taxPerChargesTotalOther, $_REQUEST['currencyId']) . '</p></div>';
		}
